...
async function generateAssets(job) {
    ...
    await db_1.db.insert(db_1.assetsTable).values({
...
async function fireWebhooks(event, job) {
    const hooks = await db_1.db.select().from(db_1.webhooksTable).where((0, drizzle_orm_1.or)((0, drizzle_orm_1.eq)(db_1.webhooksTable.event, event), (0, drizzle_orm_1.eq)(db_1.webhooksTable.event, "job.all")));
    const payload = {
        event,
    ...
    await Promise.allSettled(hooks.map(async (hook) => {
        ...
        await db_1.db.update(db_1.webhooksTable)
...
async function openAttempt(jobId, now) {
    const [existing] = await db_1.db
        ...
        .where((0, drizzle_orm_1.eq)(db_1.jobAttemptsTable.jobId, jobId));
    await db_1.db.insert(db_1.jobAttemptsTable).values({
        id: (0, crypto_1.randomUUID)(),
        jobId,
...
async function closeAttempt(jobId, result, now) {
    const [open] = await db_1.db
        ...
        .from(db_1.jobAttemptsTable)
        .where((0, drizzle_orm_1.eq)(db_1.jobAttemptsTable.jobId, jobId))
        .orderBy((0, drizzle_orm_1.desc)(db_1.jobAttemptsTable.attemptNumber))
        .limit(1);
    if (open && !open.completedAt) {
        await db_1.db.update(db_1.jobAttemptsTable)
            .set({ completedAt: now, durationMs: now.getTime() - open.startedAt.getTime(), result })
            .where((0, drizzle_orm_1.eq)(db_1.jobAttemptsTable.id, open.id));
    }
}
// ── Crash recovery ────────────────────────────────────────────────────────────
async function recoverStuckJobs() {
    const cutoff = new Date(Date.now() - JOB_TIMEOUT_MS);
    const stuck = await db_1.db
        .select({ id: db_1.jobsTable.id })
        .from(db_1.jobsTable)
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(db_1.jobsTable.status, "processing"), (0, drizzle_orm_1.lt)(db_1.jobsTable.lockedAt, cutoff)));
    if (stuck.length === 0)
        return;
    const ids = stuck.map((r) => r.id);
    await db_1.db.update(db_1.jobsTable)
        .set({ status: "queued", progress: 0, lockedAt: null, lockedBy: null, updatedAt: new Date() })
        .where((0, drizzle_orm_1.inArray)(db_1.jobsTable.id, ids));
...
async function claimQueuedJobs(slots) {
    ...
    const claimed = await db_1.db
        .update(db_1.jobsTable)
        .set({ status: "processing", progress: 0, lockedAt: now, lockedBy: WORKER_ID, updatedAt: now })
        .where((0, drizzle_orm_1.inArray)(db_1.jobsTable.id, db_1.db
        .select({ id: db_1.jobsTable.id })
        .from(db_1.jobsTable)
        .where((0, drizzle_orm_1.eq)(db_1.jobsTable.status, "queued"))
        .orderBy((0, drizzle_orm_1.desc)(db_1.jobsTable.priority), (0, drizzle_orm_1.asc)(db_1.jobsTable.createdAt))
        .limit(slots)))
        .returning();
    return claimed;
}
...
async function processTick() {
    ...
    const processingJobs = await db_1.db
        .select()
        .from(db_1.jobsTable)
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(db_1.jobsTable.status, "processing"), (0, drizzle_orm_1.eq)(db_1.jobsTable.lockedBy, WORKER_ID)));
    for (const job of processingJobs) {
        ...
        if (cancelRequested.has(job.id)) {
            ...
            await db_1.db.update(db_1.jobsTable)
                ...
                .where((0, drizzle_orm_1.eq)(db_1.jobsTable.id, job.id));
        ...
        if (newProgress >= 100) {
            ...
            const [completed] = await db_1.db.update(db_1.jobsTable)
